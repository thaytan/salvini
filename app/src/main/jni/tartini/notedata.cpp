...
NoteData::NoteData(Channel *channel_, Settings *settings_)
{
  channel = channel_;
  settings = settings_;
  maxLogRMS = settings->dBFloor();
...
NoteData::NoteData(Channel *channel_, Settings *settings_, int startChunk_, AnalysisData *analysisData)
{
  channel = channel_;
  settings = settings_;
  ...
  maxPurity = analysisData->volumeValue(maxLogRMS);
  _volume = 0.0f;
...
void NoteData::addData(AnalysisData *analysisData, float periods)
{
  ...
  maxPurity = MAX(maxPurity, analysisData->volumeValue(dbFloor));
  _volume = MAX(_volume, dB2Normalised(analysisData->logrms(), dbFloor));
  _numPeriods += periods; //sum up the periods
  //_periodOctaveEstimate = analysisData->periodOctaveEstimate; //overwrite the old estimate
  _avgPitch = bound(freq2pitch(avgFreq()), 0.0, settings->topPitch());
}
