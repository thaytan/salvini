...
double(*amp_mode_func[NUM_AMP_MODES])(double, double) =
    { &dB2Normalised, &dB2Normalised, &same, &oneMinus, &same, &same, &same };
double(*amp_mode_inv_func[NUM_AMP_MODES])(double, double) =
    { &normalised2dB, &normalised2dB, &same, &oneMinus, &same, &same, &same };

AnalysisData::AnalysisData() : settings(NULL)
{
}

AnalysisData::AnalysisData(Settings *_settings) : settings(_settings)
{
...
void AnalysisData::calcScores()
{
  ...
  double dbFloor = settings->dBFloor();
  int j;
  //double temp = 0.0;
  for(j=0; j<NUM_AMP_MODES-2; j++) {
    a[j] = bound(((*amp_mode_func[j])(values[j], dbFloor) - (*amp_mode_func[j])(settings->ampThreshold(j,0), dbFloor))
            / ((*amp_mode_func[j])(settings->ampThreshold(j,1), dbFloor) - (*amp_mode_func[j])(settings->ampThreshold(j,0), dbFloor)), 0.0, 1.0);
  }
/*
  bool aYes = false;
  bool aNo = false;
...
float AnalysisData::clarity(float ceil, float floor)
{
  /*
    Long form:
    
      AnalysisData *data = dataAtChunk(j);
      float err = data->correlation();
      float vol = dB2Normalised(data->logrms(), rmsCeiling, rmsFloor);
      float clarity = err * vol;

  */
  return correlation() * dB2Normalised(logrms(), ceil, floor);
};
