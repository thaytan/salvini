...
MyTransforms::MyTransforms(GlobalData *_gdata) : gdata (_gdata)
{
  beenInit = false;
}
...
void MyTransforms::init(int n_, int k_, double rate_, /*float threshold_, */bool equalLoudness_, int numHarmonics_)
{
  ...
  dataTemp = g_new (float, n);
  dataTime = g_new (float, n);
  dataFFT  = g_new (GstFFTF32Complex, n);
  autocorrTime = g_new (float, size);
  autocorrFFT  = g_new (GstFFTF32Complex, size);
  hanningCoeff  = g_new (float, n);

  planAutocorrTime2FFT = gst_fft_f32_new (size, FALSE);
  planAutocorrFFT2Time = gst_fft_f32_new (size, TRUE);
      // fftwf_plan_r2r_1d(size, autocorrFFT, autocorrTime, FFTW_HC2R, myFFTMode);
  ...
  planDataTime2FFT = gst_fft_f32_new (n, FALSE);
  planDataFFT2Time = gst_fft_f32_new (n, TRUE);

  harmonicsAmpLeft = g_new (float, numHarmonics);
  harmonicsPhaseLeft = g_new (float, numHarmonics);
  harmonicsAmpCenter = g_new (float, numHarmonics);
  harmonicsPhaseCenter = g_new (float, numHarmonics);
  harmonicsAmpRight = g_new (float, numHarmonics);
  harmonicsPhaseRight = g_new (float, numHarmonics);
...
{
  if(beenInit) {
    g_free (harmonicsAmpLeft);
    g_free (harmonicsPhaseLeft);
    g_free (harmonicsAmpCenter);
    g_free (harmonicsPhaseCenter);
    g_free (harmonicsAmpRight);
    g_free (harmonicsPhaseRight);

    gst_fft_f32_free (planDataFFT2Time);
    gst_fft_f32_free (planDataTime2FFT);
    gst_fft_f32_free (planAutocorrFFT2Time);
    gst_fft_f32_free (planAutocorrTime2FFT);
    g_free (hanningCoeff);
    g_free (autocorrFFT);
    g_free (autocorrTime);
    g_free (dataFFT);
...
double MyTransforms::autocorr(float *input, float *output)
{
  ...
  //Do a forward FFT
  gst_fft_f32_fft (planAutocorrTime2FFT, autocorrTime, autocorrFFT);
  ...
  for(int j=1; j<size/2; j++) {
    autocorrFFT[j].r = sq(autocorrFFT[j].r) + sq(autocorrFFT[j].i);
    autocorrFFT[j].i = 0.0f;
  }

  //Do an inverse FFT
  gst_fft_f32_inverse_fft (planAutocorrFFT2Time, autocorrFFT, autocorrTime);
...
double MyTransforms::autoLogCorr(float *input, float *output)
{
  ...
  //calculate the (real*real + ima*imag) for each coefficient
  autocorrFFT[0].r = (sq(autocorrFFT[0].r));
  autocorrFFT[0].i = (sq(autocorrFFT[0].i));
  for(int j=1; j<size/2; j++) {
    autocorrFFT[j].r = (sq(autocorrFFT[j].r) + sq(autocorrFFT[j].i));
    autocorrFFT[j].i = 0.0f;
  }
...
double MyTransforms::nsdf(float *input, float *output)
{
  ...
  if(gdata->analysisMode() == MPM || gdata->analysisMode() == MPM_MODIFIED_CEPSTRUM) { //nsdf
    for(int j=0; j<k; j++) {
...
void MyTransforms::calculateAnalysisData(/*float *input, */int chunk, Channel *ch/*, float threshold*/)
{
  ...
  analysisData.maxIntensityDB() = linear2dB(fabs(*std::max_element(curInput, curInput+n, absoluteLess<float>())), gdata->dBFloor());
  ...
  if(gdata->doingFreqAnalysis() && (ch->firstTimeThrough() || gdata->doingActiveAnalysis())) {
    ...
    double logrms = linear2dB(nsdf(dataTime, ch->nsdfData.begin()) / double(n), gdata->dBFloor()); /**< Do the NSDF calculation */
    ...
    if(analysisData.periodEstimates.empty()) { //no period found
      ...
      //calc the periodDiff
      if(chunk > 0 && prevAnalysisData->highestCorrelationIndex >= 0) {
      ...
      if(!analysisData.done) {
        if(gdata->analysisMode() == MPM_MODIFIED_CEPSTRUM) {
...
#if 0 /* JS - disable pitch/Note tracking for RTTA */
  ...
  if(!analysisData.done) {
    ...
    analysisData.logrms() = linear2dB(rms / float(n), gdata->dBFloor());
    analysisData.calcScores();
    analysisData.done = true;
  }
...
void MyTransforms::doChannelDataFFT(Channel *ch, float *curInput, int chunk)
{
  ...
  gst_fft_f32_fft (planDataTime2FFT, dataTime, dataFFT);
  int nDiv2 = n/2;
  ...
  for(int j=1; j<nDiv2; j++) {
    sqValue = sq(dataFFT[j].r) + sq(dataFFT[j].i);
  ...
  sqValue = sq(dataFFT[0].r) + sq(dataFFT[nDiv2].r);
  ...
  if(gdata->analysisMode() == MPM_MODIFIED_CEPSTRUM) {
    ...
    //dataFFT[0] = log10(1.0 + sqrt(sq(dataFFT[0]) + sq(dataFFT[nDiv2])));
    dataFFT[0].r = ch->fftData2[0];
    //dataFFT[0] = 0.0;
    dataFFT[nDiv2].r = 0.0;
    gst_fft_f32_inverse_fft (planDataFFT2Time, dataFFT, dataTime);
...
void MyTransforms::calcHarmonicAmpPhase(float *harmonicsAmp, float *harmonicsPhase, int binsPerHarmonic)
{
  ...
  for(int j=0; j<numHarmonics; j++) {
    ...
    if(harmonic < n) {
      harmonicsAmp[j] = sqrt(sq(dataFFT[harmonic].r) + sq(dataFFT[harmonic].i));
      harmonicsPhase[j] = atan2f(dataFFT[harmonic].i, dataFFT[harmonic].r);
    } else {
      harmonicsAmp[j] = 0.0;
