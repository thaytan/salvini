...
Channel::Channel(GlobalData *_gdata, SoundFile *parent_, int size_, int k_) : gdata (_gdata), lookup()
{
  setParent(parent_);
  int sampleRate = parent->rate();

  /* Keep at most ~60s of analysis vectors. FIXME: Make configurable */
  lookup_size = (int)(60000 / (size_ * 1000 / sampleRate));
  //printf ("keeping %d chunks at most of size %d samples\n", lookup_size, size_);
  ...
  noteIsPlaying = false;
  //setThreshold(0.97f);
  setIntThreshold(gdata->getAnalysisThreshold());
  //estimate = 0.0;
  freq = 0.0;
  int filterIndex = 2;
...
//
//  Calculate RTTA data for the chunk at i
//
void Channel::calculateRTTA(int i)
{
  if (dataAtChunk(i)->clarity(rmsCeiling, rmsFloor) >
      gdata->getClarityThreshold() &&
      dataAtChunk(i)->logrms() >
      gdata->getVolumeThreshold()) {

    gdata->rtta_data->addInfo(dataAtChunk(i)->pitch);
...
void Channel::processNewChunk(FilterState *filterState)
{
  ...
  lookup.push_back(AnalysisData(gdata->getSettings()));
  lookup.back().filterState = *filterState;
  if (lookup.size() > lookup_size)
    lookup.pop_front();

  int i = int(lookup.size())-1;
  parent->myTransforms.calculateAnalysisData(i, this);
  calculateRTTA(i);
}
...
float Channel::averagePitch(int begin, int end)
{
  ...
  for (int i = begin; i < end; i++) {
    ...
    weight = window_weight * data->correlation() * dB2Linear(data->logrms(), gdata->dBFloor());
...
void Channel::backTrackNoteChange(int chunk) {
  ...
  int first = MAX(chunk - (int)ceil(longTime/timePerChunk()), currentNote->startChunk()/*+5*/);
  ...
  currentNote->setEndChunk(largestDiffChunk);
  currentNote->recalcAvgPitch();
...
void Channel::processNoteDecisions(int chunk, float periodDiff)
{
  ...
  if(noteIsPlaying) {
      ...
      addToNSDFAggregate(dB2Linear(analysisData.logrms(), gdata->dBFloor()), periodDiff);
...
{
  AnalysisData *analysisData = dataAtChunk(chunk);
  /* JS: Disable note data tracking, we only want RTTA,
   * and large_vectors can't be truncated so grow infinitely */
  //noteData.push_back(NoteData(this, gdata->getSettings(), chunk, analysisData));
...
*/
  ...
  if(gdata->analysisMode() == MPM || gdata->analysisMode() == MPM_MODIFIED_CEPSTRUM) {
    /*
    //choose a cutoff value based on the highest value and a relative threshold
...
#if 0
RTTAinfo myInfo[RTTA_NOTE_RANGE] = {
  {"C3", 47.5},
  {"C#3", 48.5},
  {"D3", 49.5},
  {"D#3", 50.5},
...
void Channel::exportChannel(QTextStream &out)
{
    out << "        Time(secs) Pitch(semi-tones)       Volume(rms)	Clarity" << endl;
    out << qSetFieldWidth(18);
    for(int j=0; j<totalChunks(); j++)
      if (dataAtChunk(j)->pitch) {
	out << timeAtChunk(j) <<  dataAtChunk(j)->pitch << dataAtChunk(j)->logrms() << dataAtChunk(j)->clarity(rmsCeiling, rmsFloor) << endl;
      };
}
#endif
