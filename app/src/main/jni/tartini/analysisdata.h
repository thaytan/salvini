...
public: 
  ...
  //bool isValid();
  AnalysisData();
  AnalysisData(Settings *_settings);
  void calcScores();
  float clarity(float ceil, float floor);
  ...
  float volumeValue(double dbFloor) { return (dB2Normalised(values[AMPLITUDE_RMS], dbFloor) + values[AMPLITUDE_CORRELATION]-1.0f) * 0.2; }
